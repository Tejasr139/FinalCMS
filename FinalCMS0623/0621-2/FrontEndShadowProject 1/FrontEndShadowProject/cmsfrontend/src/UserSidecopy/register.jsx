...
//---------------------- PROPS WORKING_________________________________________
...
function Register({ onUpdateBy }) { // Receive the onUpdateBy prop
  const [user, setUser] = useState({
    userName: '',
    password: '',
    email: '',
  ...
  const handleSubmit = (event) => {
    ...
    .then((data) => {
        setNotification('User registered successfully!');
        onUpdateBy(user.userName); // Call onUpdateBy with the username
        navigate('/login', { state: { updatedBy: user.userName } });
      })
    .catch((error) => {
        setNotification(`Error registering user: ${error.message}`);
      });
